...
To bridge this gap, we implement \texttt{ModePolicy}, a pre-episode execution-mode router. Before tool registration, \texttt{ModePolicy} analyzes the task instruction using instruction-only structural features (e.g., multi-file dependency hints, shell pipeline keywords) and a lightweight complexity probe. We train a logistic-regression classifier over TF-IDF tokens plus these structural features, using only the 88 frozen Terminal-Bench task instructions and their per-task preferred mode (\texttt{batch} vs \texttt{native}) as supervision. Training uses all concordant tasks plus 70\% of discordant tasks; the remaining 30\% of discordant tasks are a held-out gate set. No outcome, reward, cost, or verifier field is used as a feature. The policy maps each episode to an execution mode $m \in \{\text{native}, \text{batch\_available}\}$, registering \texttt{batch\_queue} only when predicted to be beneficial.
...
\begin{table}[ht]
\centering
\caption{ModePolicy Strategy Evaluation on 24-Task Stratified Terminal-Bench Slice (offline, frozen outcomes).}
\label{tab:mode_router}
\small
\input{tables/mode_router}
\end{table}

As shown in Table~\ref{tab:mode_router}, the instruction-trained classifier achieves a 70.8\% pass@1 resolution on the stratified slice, outperforming both Always Native ($66.7\%$) and the keyword-only heuristic ($50.0\%$), while reducing total execution cost below the Always Native baseline (\$10.42 vs \$12.21). The gap between the classifier and the oracle upper bound (95.8\%) motivates scaling adaptive routing to the full benchmark.
