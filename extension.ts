...
export function registerBatchedQueueExtension(
	...
	pi.registerTool({
		...
		description:
			...
			"Provide either:\n" +
			"- `objective`: the executor model plans the batch (fast/cheap model)\n" +
			"- `actions`: the driver model supplies a pre-planned batch directly\n\n" +
			"Action types: read_lines, grep_pattern, execute_bash, apply_diff.\n" +
			"Fast-fail: batch halts on first non-zero exit or validation failure.\n" +
